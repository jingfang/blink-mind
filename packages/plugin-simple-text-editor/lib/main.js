...
var SimpleTextEditor = /** @class */ (function (_super) {
    ...
    SimpleTextEditor.prototype.render = function () {
        ...
        var _b = this.getCustomizeProps(), readOnly = _b.readOnly, refKeyPrefix = _b.refKeyPrefix, placeholder = _b.placeholder, style = _b.style;
        log(readOnly);
...
var TopicContentEditor = /** @class */ (function (_super) {
    ...
    TopicContentEditor.prototype.onClickOutSide = function (e) {
        ...
        controller.run('operation', __assign(__assign({}, this.props), { opType: core.OpType.SET_TOPIC_CONTENT, data: this.state.content }));
        controller.run('operation', __assign(__assign({}, this.props), { opType: core.OpType.FOCUS_TOPIC, focusMode: core.FocusMode.NORMAL }));
    };
...
function SimpleTextEditorPlugin() {
    return {
        getTopicTitle: function (props) {
            var model = props.model, controller = props.controller, topicKey = props.topicKey;
            var topic = model.getTopic(topicKey);
            var block = topic.getBlock(core.BlockType.CONTENT).block;
            var txt = controller.run('serializeBlockData', __assign(__assign({}, props), { block: block }));
            return txt;
        },
